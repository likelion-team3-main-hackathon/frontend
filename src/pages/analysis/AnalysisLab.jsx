...
export default function AnalysisLab({ onNavigate }) {
  ...
  const metrics = useMemo(() => {
    ...
    const dayDivisor = period === 'weekly' ? 7 : 1
    ...
    return [
      { id: 'meal', label: '식단', score: meals.length ? Math.min(100, Math.round(55 + meals.length / dayDivisor * 9)) : 64, note: meals.length ? `식단 ${meals.length}회 기록` : '단백질 미달' },
      { id: 'exercise', label: '운동', score: exercises.length ? Math.min(100, Math.round(62 + exercises.length / dayDivisor * 8)) : 78, note: exercises.length ? `운동 ${exercises.length}개 완료` : '하체 공백' },
  ...
  return <section className="analysis-lab-page">
    <div className="analysis-lab-scroll">
      ...
      <article className="analysis-summary-card"><header><h2>종합 분석</h2><span>{period === 'weekly' ? `${weekStart.getMonth() + 1}/${weekStart.getDate()}–${today.getMonth() + 1}/${today.getDate()} 기준` : '오늘 기준'}</span></header><p>{summary}</p><ol><li><b>1</b>{goal || '단백질 섭취량을 목표에 맞게 보완'}</li><li><b>2</b>{caution || '오후 운동으로 활동 공백 보완'}</li><li><b>3</b>나트륨 2.3g 이하 유지 · 외식 조절</li></ol><button type="button" onClick={() => onNavigate?.('ai-chat')}>루틴에 반영하기</button></article>

      <h2 className="lab-title">검사실 4</h2>
      <div className="lab-grid">{metrics.slice(0, 4).map((metric) => <article className={metric.id === 'meal' ? 'clickable' : ''} role={metric.id === 'meal' ? 'button' : undefined} tabIndex={metric.id === 'meal' ? 0 : undefined} onClick={() => metric.id === 'meal' && onNavigate?.('meal-analysis')} onKeyDown={(event) => { if (metric.id === 'meal' && event.key === 'Enter') onNavigate?.('meal-analysis') }} key={metric.id}><header><strong>{metric.label}</strong>{metric.warning ? <em>주의 {metric.warning}</em> : <b>{metric.score}<small>/100</small></b>}</header><p>{metric.note}</p><div><i style={{ width: `${metric.score}%` }} /></div></article>)}</div>
