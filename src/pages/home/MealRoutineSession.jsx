...
function initialFoods(item) {
  ...
  if (Array.isArray(supplied) && supplied.length) return supplied.map((food, index) => ({ id: food.id || `food-${index}`, ...food }))
  if (Array.isArray(item.menu) && item.menu.length) return item.menu.map((name, index) => ({ id: `menu-${index}`, name, calories: 0, carbs: 0, protein: 0, fat: 0 }))
  return [{ id: `routine-food-${item.id}`, name: item.title, calories: 0, carbs: 0, protein: 0, fat: 0 }]
}
...
export default function MealRoutineSession({ item, onDecision, onClose }) {
  ...
  if (view === 'detail' && selectedFood) return <MealFoodDetail food={selectedFood} photoUrl={photo?.url || ''} routineId={item.routineId || item.id} onBack={() => setView('list')} onSave={(updatedFood) => { setFoods((current) => current.map((food) => food.id === updatedFood.id ? updatedFood : food)); setView('list') }} onDelete={() => { setFoods((current) => current.filter((food) => food.id !== selectedFood.id)); setSelectedFoodId(null); setView('list') }} />

  return <section className="meal-session-page">
    <header className="meal-session-header"><button type="button" onClick={onClose} aria-label="뒤로 가기">‹</button><h1>{item.routineTitle || '식단 루틴'} {item.dayNumber || 1}일차</h1></header>
    <button type="button" className="meal-photo-card" onClick={() => setView('camera')} style={photo?.url ? { backgroundImage: `url(${photo.url})` } : undefined}>{!photo?.url && <><img src={cameraIcon} alt="" /><small>사진 인증 및 분석</small></>}</button>
    ...
    <button type="button" className="meal-confirm-button" disabled={isSaving || isAnalyzing || foods.length === 0} onClick={confirm}>{isAnalyzing ? '분석 중…' : isSaving ? '저장 중…' : '확인'}</button>
  </section>
}
